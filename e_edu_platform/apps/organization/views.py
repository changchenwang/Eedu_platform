...
class AddFavView(View):
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated:
            #判断用户登录状态
            return HttpResponse('{"status":"fail", "msg":"The users have not login"}', content_type='application/json')
        ...
        if exist_records:
            ...
            if int(fav_id) > 0 and int(fav_type) > 0:
                ...
                return HttpResponse('{"status":"success", "msg":"Already favorite"}', content_type='application/json')
            else:
